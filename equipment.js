...
import { platformsOf, solidProps } from "./maps.js";
...
function armorDebrisColor(fighter) {
  // Match retractable plate faction tint used in rendering.
...
export function spawnBrokenArmorDebris(game, fighter) {
  ...
  for (let i = 0; i < specs.length; i++) {
    const spec = specs[i];
    const burst = 140 + (i % 3) * 35;
    const ang = (-Math.PI / 2) + (Math.random() - 0.5) * 1.4 + facing * 0.15;
    game.armorDebris.push({
      kind: spec.kind,
      x: cx + spec.ox,
      y: cy + spec.oy,
      vx: Math.cos(ang) * burst + (fighter.vx || 0) * 0.25,
      vy: Math.sin(ang) * burst - 90 - Math.random() * 80 + (fighter.vy || 0) * 0.15,
  ...
  // Brief shatter flash (existing short-lived debris effect).
  game.effects ||= [];
  game.effects.push({
    type: "debris",
    x: cx,
...
/** Integrate broken armor pieces; they rest on platforms for the whole match. */
export function tickArmorDebris(game, dt) {
